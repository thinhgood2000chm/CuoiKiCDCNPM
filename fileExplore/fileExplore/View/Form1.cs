   ...
   // sửa nội dung file txt trong chạy ngầm ( đạt ) 
    public partial class Form1 : Form
    {
        ...
        public void getAllFileInDriver()
        {
            ...
            DirectoryInfo info = new DirectoryInfo(@"C:\GAME-ON\Test");
            if (IsHandleCreated)
            {
        ...
        private void CheckChangeFile()
        {
            try
            {
                ...
                using (StreamReader file = new StreamReader($"{nameDriver}\\data_key\\ChangeLog.txt"))
                {
                    Debug.WriteLine("da vao " + $"{nameDriver}\\data_key\\ChangeLog.txt");
                    string ln;
                    while ((ln = file.ReadLine()) != null)
                    {
                        if (ln.Contains("OnCreated"))
                        {
                    ...
                    file.Close();
                    File.WriteAllText($"{nameDriver}\\data_key\\ChangeLog.txt", "");
                }
